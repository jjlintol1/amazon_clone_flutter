import 'package:amazon_clone/model/order_requests_model.dart';
...
class CloudFirestoreClass {
  ...
  Future<String> uploadProductToDatabase({
    ...
    if (image != null && productName != "" && rawCost != "") {
      try {
        ...
        .doc(uid).set(product.getJson());
        output = "success";
      } catch (e) {
        output = e.toString();
      }
    } else {
      output = "Please ensure all fields are not empty";
    }
  ...
  Future<List<Widget>> getProductsFromDiscount(int discount) async {
    ...
    QuerySnapshot<Map<String, dynamic>> snap = await firebaseFirestore.collection("products")
    .where(
      "discount",
      isEqualTo: discount
    ).get();
    for (int i = 0; i < snap.docs.length; i++) {
      ...
      ProductModel model = ProductModel.getModelFromJson(json: (docSnap.data() as dynamic));
      children.add(
  ...
  Future buyAllItemsInCart({
    ...
    QuerySnapshot<Map<String, dynamic>> snapshot = await firebaseFirestore.collection("users")
    .doc(firebaseAuth.currentUser!.uid)
    .collection("cart")
    .get();
  ...
  Future changeAverageRating({
    ...
    DocumentSnapshot snapshot = await firebaseFirestore.collection("products")
    .doc(productUid)
    .get();
